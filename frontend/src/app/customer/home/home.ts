...
@Component({
  ...
  template: `
    <div class="space-y-10 animate-fade-in">
      <div class="bg-gradient-to-br from-[#008269] to-[#005a4b] rounded-[2.5rem] p-12 text-white shadow-2xl relative overflow-hidden">
        <div class="relative z-10 max-w-2xl">
          ...
          <div class="flex flex-wrap gap-4">
            <button (click)="action.emit('register')" class="px-8 py-4 bg-white text-[#008269] rounded-2xl font-black shadow-xl hover:scale-105 transition-all">
              Register a New Complaint
            </button>
            <button (click)="action.emit('list')" class="px-8 py-4 bg-[#008269]/20 backdrop-blur-md border border-white/30 text-white rounded-2xl font-black hover:bg-white/10 transition-all">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div class="bg-white p-8 rounded-[2rem] shadow-xl border border-gray-100 flex flex-col items-center text-center group hover:-translate-y-2 transition-all">
           <div class="w-16 h-16 bg-red-50 text-[#008269] rounded-2xl flex items-center justify-center text-2xl mb-6 group-hover:bg-[#008269] group-hover:text-white transition-all">
